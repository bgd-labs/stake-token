...
contract InvariantHandler is CommonBase, StdCheats, StdUtils {
  ...
  constructor(IStakeToken stakeToken) {
  ...
  function stake(uint256 assets, uint256 actorIndexSeed) external useActor(actorIndexSeed) {
    ...
    IERC20(_stakeToken.asset()).forceApprove(address(_stakeToken), assets);
    _stakeToken.stake(currentActor, assets);
  ...
  function slash(uint256 assets) external {
    vm.startPrank(OwnableUpgradeable(address(_stakeToken)).owner());
    uint256 slashed = _stakeToken.slash(OwnableUpgradeable(address(_stakeToken)).owner(), assets);
    ghost_sumOfStakedAssets -= slashed;
    vm.stopPrank();
  }
